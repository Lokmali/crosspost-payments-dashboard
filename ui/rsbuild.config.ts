...
import { HydrateContainerShimPlugin } from "./build/hydrate-container-shim-plugin.ts";
import { buildStandaloneRuntimeConfig } from "./build/standalone-runtime-config.ts";
...
const lifecycleScript = process.env.npm_lifecycle_event ?? "";
const isStandalone =
  lifecycleScript === "dev:standalone" || process.env.STANDALONE_DEV === "true";
const devUiPort = Number(process.env.STANDALONE_PORT ?? 3003);
const devProxyHostPort = Number(process.env.STANDALONE_PROXY_HOST ?? 3000);
...
const isDev = !shouldDeploy;

const uiSharedDeps = {
  react: {
    version: getInstalledVersion("react", pkg.dependencies.react),
    ...SHARE_DEFAULTS,
    eager: isDev,
  },
  "react-dom": {
...
function createClientConfig(command: "dev" | "build" | "preview") {
  const useDevAppShell = command === "dev" || isStandalone;
  const devRuntimeConfig = useDevAppShell
    ? buildStandaloneRuntimeConfig({
        port: devUiPort,
        proxyHostPort: devProxyHostPort,
      })
    : null;
  ...
  return defineConfig({
    ...
    source: {
      entry: {
        index: useDevAppShell ? "./src/dev-entry.tsx" : "./src/hydrate.tsx",
      },
      define: {
        ...(devRuntimeConfig
          ? { __STANDALONE_RUNTIME_CONFIG__: JSON.stringify(devRuntimeConfig) }
          : {}),
        "import.meta.env.APP_NAME": JSON.stringify(bosConfig.domain),
        "import.meta.env.APP_ACCOUNT": JSON.stringify(bosConfig.account),
        "import.meta.env.VITE_PIKESPEAK_API_KEY": JSON.stringify(
          process.env.VITE_PIKESPEAK_API_KEY ?? "",
        ),
      },
    },
    resolve: {
    ...
    server: {
      port: isServerBuild ? 3004 : devUiPort,
      printUrls: ({ urls }) => urls.filter((url) => url.includes("localhost")),
      ...(useDevAppShell
        ? {
            proxy: {
              "/api": {
                target: `http://localhost:${devProxyHostPort}`,
                changeOrigin: true,
              },
            },
          }
    ...
    tools: {
      rspack: {
        ...
        plugins: [
          ...
          new HydrateContainerShimPlugin(),
        ],
      },
    },
    output: {
...
export default defineConfig(({ command }) => {
  ...
  return createClientConfig(command);
});
